...
public class OsgiProject extends Project {
    ...
    public synchronized void removeTask(JavaAnalysis task) {
    ...
    public synchronized void receiveTask(JavaAnalysis task) {
        String name = task.getName();
        ...
        Map<String, Trgt<?>> inputs = task.getConsumerMap();
        cachedNameToInputs.put(name, inputs);
        Map<String, Trgt<?>> outputs = task.getProducerMap();
