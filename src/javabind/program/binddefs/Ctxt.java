...
import javabind.analyses.contexts.ContextsAnalysis;
import soot.Unit;
 ...
 * methods (see {@link ContextsAnalysis}).
 *
	...
	public String toString(boolean asCtxtObj) {
        ...
        for (int i = asCtxtObj ? 1 : 0; i < elems.length; i++) {
			...
			builder.append(inst == null ? "null"
						   : (inst instanceof Unit ? BindUtils.unitToString((Unit) inst) : inst.toString()));
		}
		builder.append(']');
		if (asCtxtObj) {
			builder.append(':');
