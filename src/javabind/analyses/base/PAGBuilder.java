...
{
    public static int persistOrder; // 0 - dont care, 1 - write out the order, 2 - read in the stored order.
...
//    static String gInstallAPK = "INSTALL_APK";
        ...
        void handleStmt(Stmt s)
        {
            if(s.containsInvokeExpr()){
                ...
                if(domM.contains(callee)){
                    ...
                    else if(BindUtils.isQuasiStaticInvk(s)){
    ...
    void populateMethods()
    {
        ...
        ArrayList<SootMethod> orderedMeths = new ArrayList<SootMethod>();
        if (persistOrder == 1) {
            ArrayList<String> methSig = new ArrayList<String>();
            while (mIt.hasNext()) {
                SootMethod m = mIt.next();
                orderedMeths.add(m);
                methSig.add(m.getSignature());
            }
            dump(methSig, "methods.txt");
            mIt = orderedMeths.iterator();
        } else if (persistOrder == 2) {
            ArrayList<String> methSig = load("methods.txt");
            for (String msig : methSig) {
                // If jimple reserved words occur as field/method names, they are quoted.
                // See https://groups.google.com/forum/#!topic/soot-list/iAAioHOUKZE
                // It is likely that getMethod might handle this because it gets by signature.
                SootMethod m = program.scene().getMethod(msig);
                orderedMeths.add(m);
            }
            mIt = orderedMeths.iterator();
        }
    ...
    void populateFields()
    {
        ...
        } else if (persistOrder == 1) {
            ArrayList<String> fsig = new ArrayList<String>();
            for(SootClass klass : program.getClasses()){
                for(SootField field : klass.getFields()){
                    domF.add(field);
                    fsig.add(klass.toString() + "##" + field.toString());
                }
            }
            dump(fsig, "fields.txt");
        } else if (persistOrder == 2) {
            ArrayList<String> fSig = load("fields.txt");
            for (String f : fSig) {
                String[] parts = f.split("##");
                SootClass c = program.scene().getSootClass(parts[0].trim());
                String[] nParts = parts[1].split(" ");
                String fldName = nParts[2].substring(0, nParts[2].length() - 1);
                fldName = nParts[1] + " " + nParts[2].substring(0, nParts[2].length() - 1);
                // If jimple reserved words occur as field/method names, they are quoted.
                // See https://groups.google.com/forum/#!topic/soot-list/iAAioHOUKZE
                if (fldName.startsWith("'")) fldName = fldName.substring(1, fldName.length() - 1);
                SootField fld = c.getFieldUnsafe(fldName);
                domF.add(fld);
            }
        }
        domF.save();
    ...
    void populateTypes()
    {
        ...
        ArrayList<Type> orderedTypes = new ArrayList<Type>();

        if (persistOrder == 1) {
            ArrayList<String> typesSig = new ArrayList<String>();
            while (typesIt.hasNext()) {
                Type typ = typesIt.next();
                orderedTypes.add(typ);
                typesSig.add(typ.toString());
            }
            dump(typesSig, "types.txt");
            typesIt = orderedTypes.iterator();
        } else if (persistOrder == 2) {
            ArrayList<String> typesSig = load("types.txt");
            HashMap<String,Type> str2Typ = new HashMap<String,Type>();
            while(typesIt.hasNext()) {
                Type t = typesIt.next();
                str2Typ.put(t.toString(), t);
            }
            for (String tsig : typesSig) {
                Type typ = str2Typ.get(tsig);
                orderedTypes.add(typ);
            }
            typesIt = orderedTypes.iterator();
        }
    ...
    void populateDomains(List<MethodPAGBuilder> mpagBuilders)
    {
        ...
        if (persistOrder == 1) {
            ...
            dump(methSig, "reachM.txt");
            mIt = orderedMeths.iterator();
        } else if (persistOrder == 2) {
            ArrayList<String> methSig = load("reachM.txt");
            for (String msig : methSig) {
                SootMethod m = prog.scene().getMethod(msig);
                orderedMeths.add(m);
            }
            mIt = orderedMeths.iterator();
        }
	...
	*/
    ...
    public void run()
    {
        ...
        persistOrder = Integer.getInteger("shord.persist.order", 0);
