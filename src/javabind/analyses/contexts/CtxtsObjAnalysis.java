package javabind.analyses.contexts;
...
import javabind.analyses.base.DomH;
import javabind.analyses.base.DomI;
import javabind.analyses.base.DomM;
import javabind.analyses.base.DomV;
import soot.Scene;
import soot.Unit;
...
@Chord(name = "ctxts-obj-java",
       ...
       produces = { "C", "CC", "CH", "CI", "CtxtInsMeth", "CM", "CMap"},
       namesOfTypes = { "C" },
       types = { DomC.class }
)
public class CtxtsObjAnalysis extends JavaAnalysis 
{
	...
	public static int K;
	public static boolean ctxtInsens;
    ...
    private ProgramRel relCMap;

	private NumberedSet stubs;

    public void run() 
	{
    	K = Integer.getInteger("shord.kobj.k", 0);
    	ctxtInsens = Boolean.getBoolean("shord.kobj.ci");
		...
		relCMap = (ProgramRel) ClassicProject.g().getTrgt("CMap");
        ...
        for(int iIdx = 0; iIdx < ItoM.length; iIdx++){
			...
			if(!BindUtils.isQuasiStaticInvk(invk))
				continue;
        ...
        ////CMap
        relCMap.zero();
        for(int cIdx = 0; cIdx < domC.size(); cIdx++){
        	if (ctxtInsens)
        		relCMap.add(domC.get(cIdx), domC.get(0));
        	else
        		relCMap.add(domC.get(cIdx), domC.get(cIdx));
        }
        relCMap.save();
    ...
    private void doAnalysis() {
		...
		while(mIt.hasNext()){
			...
			if (meth == mainMeth || meth.getName().equals("<clinit>") || treatCI(meth)) {
                ...
                if(BindUtils.isQuasiStaticMeth(meth)) {
    ...
    private Set<Ctxt> getNewCtxts(int cleIdx) { // Update contexts for this method (callee)
        ...
        if(BindUtils.isQuasiStaticMeth(meth)){//static?copy all the ctxts from its callers.
